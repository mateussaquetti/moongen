...
for time in range(DEF_PKT_NUM):
    ...
    if( vlan_id == vlan_id_map["nnp4"] ):
        ...
        # elif ((dst_host_map[src_host] == 0) or (dst_host_map[src_host] == 1)):
            ...
            pkt_exp = Ether(src=src_MAC_exp, dst=dst_MAC_exp) / IP(src=src_IP, tos=200, dst=dst_IP_exp, ttl=64, chksum=0x7ce7) / ProbeData(occup=1, nnid=5, neurondst=0, info=97, data=0000000) / ((DEF_PKT_SIZE - (HEADER_SIZE+17))*"A")
            extra_len = LEN_PROBE
            ...
            # occup=0, neurondst=14, info=126, data=1193040
            pkt_exp = Ether(src=src_MAC_exp, dst=dst_MAC_exp) / IP(src=src_IP_exp, tos=200, dst=dst_IP_exp, ttl=64, chksum=0x7ce7) / ProbeData(occup=1, neurondst=14, info=97, data=0000000) / ((DEF_PKT_SIZE - (HEADER_SIZE+17))*"A")
            extra_len = LEN_PROBE
    else:
